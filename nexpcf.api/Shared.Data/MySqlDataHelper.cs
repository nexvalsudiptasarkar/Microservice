...
namespace Shared.Data
{
    public class MySqlDataHelper
    {
        ...
        public async Task<int> ExecuteNonQuery(string strConn, string strProc, CommandType cmdType, MySqlParameter[] arrPrm, ILogger _logger)
        {
            try
            {
                ...
                int i = await ExecuteNonQueryThreadSafe(strConn, strProc, cmdType, arrPrm,_logger);
                return i;
            }
            catch (Exception err)
            {
        ...
        public async Task<int> ExecuteNonQueryThreadSafe(string strConn, string strProc, CommandType cmdType, MySqlParameter[] arrPrm, ILogger _logger)
        {
            ...
            try
            {
                ...
                using (MySqlConnection dataConnection = new MySqlConnection(strConn))
                {
                    ...
                    using (MySqlCommand icmd = dataConnection.CreateCommand())
                    {
                        ...
                        result = await icmd.ExecuteNonQueryAsync();
        ...
        public async Task<object> ExecuteScalar(string strConn, string strProc, CommandType cmdType, MySqlParameter[] arrPrm, ILogger _logger)
        {
            Tuple<MySqlCommand, MySqlConnection> tplObj = await PrepareCommand(strConn, strProc, cmdType, arrPrm, _logger);
            var obj = await tplObj.Item1.ExecuteScalarAsync();
        ...
        public async Task<object> ExecuteReader(string strConn, string strProc, CommandType cmdType, MySqlParameter[] arrPrm, ILogger _logger)
        {
            try
            {
                Tuple<MySqlCommand, MySqlConnection> tplObj = await PrepareCommand(strConn, strProc, cmdType, arrPrm, _logger);
        ...
        public async Task<DataSet> ExecuteDataSet(string strConn, string strProc, CommandType cmdType, MySqlParameter[] arrPrm, ILogger _logger)
        {
            try
            {
                /*_logger.LogInformation(string.Format("{0} :: {1} :: Start at {2}",
        ...
        private async Task<Tuple<MySqlCommand, MySqlConnection>> PrepareCommand(string strConn, string strProc, CommandType cmdType, MySqlParameter[] arrPrm, ILogger _logger)
        {
            try
            {
                /*_logger.LogInformation(string.Format("{0} :: {1} :: Start at {2}",
